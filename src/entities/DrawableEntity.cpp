...
void DrawableEntity::draw(const mat4& view_projection_matrix, const Frustum& frustum) {
    ...
    if(b_is_visible) {
        ...
        if(aabb == nullptr || aabb->is_in_frustum(frustum.view_projection * transform.get_global_model())) {
            ...
            if(b_is_selected) {
                const Shader* actual_shader = shader;
                shader = AssetManager::get_shader_ptr("flat");
                shader->use();
